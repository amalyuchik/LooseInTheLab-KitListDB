...
require_once($_SERVER['DOCUMENT_ROOT']."/kit_db/includes/classes/refill_kit_list_item.php");
...
$query_select = $select_sql->query("refill_kit_list", null, null, $_GET['book_id']);
...
foreach ($sql_execute as $current_record)
{
    ...
    {
        array_push($kit_master_array, new refill_kit_list_item($current_record));
    }
    else
    {
        ...
        {
            if ($current_record['reusable'] && $last_item->reusable_qty < $current_record['lab_product_line_refill_qty'])
            {
                $last_item->reusable_qty = $current_record['lab_product_line_refill_qty'];
                $last_item->product_reusable_price = $current_record['product_price']*$current_record['lab_product_line_refill_qty'];
            }
            elseif (!$current_record['reusable'])
            {
                $last_item->consumable_qty += $current_record['lab_product_line_refill_qty'];
                $last_item->product_consumable_price += $current_record['product_price'] * $current_record['lab_product_line_refill_qty'];
            }
...
<head>
    ...
    <title><?php echo $book_title; ?> Classroom Refill Kit List</title>
</head>
...
<div class="container">
    ...
    <section>
        <h1><?php echo $book_title; ?> Classroom Refill Kit List</h1>
        <div class="row">
